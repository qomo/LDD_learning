...
static int globalmem_init(void)
{
    ...
    proc_entry = proc_create("globalmem_proc", 0644, NULL, &globalmem_fops);
    if (proc_entry == NULL) {
        printk(KERN_INFO "globalmem_proc: Couldn't create proc entry\n");
    }
    return 0;
...
{
    remove_proc_entry("globalmem_proc", NULL);
